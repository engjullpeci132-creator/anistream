...
export default async function HomePage({ searchParams }) {
  ...
  let trending = [], airing = [], results = [], hasNext = false;

  if (q) {
    const d = await searchAnime(q, page);
    results = d.results; hasNext = d.hasNext;
  } else if (genre) {
    const d = await getByGenre(genre, page);
    results = d.results; hasNext = d.hasNext;
  } else {
    const d = await getTrending(page);
    trending = d.trending; airing = d.airing; hasNext = d.hasNext;
  }

  const hero      = trending[0] || null;
  const heroTitle = hero ? getTitle(hero) : '';
  const heroCover = hero ? getCover(hero) : '';
  const heroScore = hero ? getScore(hero) : null;

  const PT = { paddingTop: 'calc(var(--nav) + var(--gbar) + 2.5rem)' };
  ...
  return (
    <>
      ...
      {!q && !genre && hero && (
        <section style={{
          ...
          <div style={{ position: 'absolute', inset: 0, overflow: 'hidden' }}>
            ...
            <img
              src={hero.bannerImage || heroCover}
              alt={heroTitle}
          ...
          <div style={{ position: 'relative', padding: '0 clamp(1rem,5vw,4.5rem) clamp(2rem,4vh,4rem)', maxWidth: 660 }}>
            <div style={{
              ...
              background: 'rgba(230,57,80,.1)', border: '1.5px solid rgba(230,57,80,.22)',
              color: 'var(--acc2)', borderRadius: 20, padding: '5px 13px',
              fontSize: '.72rem', fontWeight: 700, letterSpacing: '.9px', textTransform: 'uppercase',
              marginBottom: '1rem',
            }}>🔥 Trending #1</div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: '.85rem', flexWrap: 'wrap' }}>
              {heroScore && <span style={{ background: 'var(--goldd)', border: '1.5px solid rgba(245,197,24,.22)', color: 'var(--gold)', borderRadius: 7, padding: '3px 9px', fontSize: '.78rem', fontWeight: 700 }}>★ {heroScore}/10</span>}
            ...
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
              <Link
                href={`/watch?id=${hero.id}&ep=1&title=${encodeURIComponent(heroTitle)}&eps=${hero.episodes || 0}`}
      ...
      <main id="main">
        ...
        {/* SEARCH */}
        {q && (
          <section className="section" style={PT}>
            <div className="sh">
              ...
              <section className="section">
                ...
                <div className="airing-grid">
                  {airing.map(a => {
                    ...
                    return (
                      <Link key={a.id}
                        href={`/watch?id=${a.id}&ep=1&title=${encodeURIComponent(t)}&eps=${a.episodes || 0}`}
      ...
      <footer className="footer">
        <div>
          <div className="footer-logo">Ani<em>Stream</em></div>
          <p>Free anime streaming powered by AniList. Streams via aniwatch-api. Zero media hosted.</p>
        </div>
        <div>
          <h4>Genres</h4>
        ...
        <div>
          <h4>Legal</h4>
          {['DMCA','Privacy','Terms'].map(l => <a key={l} href="#">{l}</a>)}
        </div>
        <div className="footer-bottom">
          AniStream does not store any media. Streams via aniwatch-api. Data from AniList. © {new Date().getFullYear()} AniStream
        </div>
      </footer>
    </>
  );
